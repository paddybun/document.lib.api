...
internal class TagApiService(ITagService tagService)
{
    ...
    {
        var res = PropertyValidator.AnyHasValue(parameters, x => x.Id, x => x.Name);
        
        if (PropertyValidator.AllHaveValue(parameters, x => x.Id))
        {
        ...
        if (PropertyValidator.AllHaveValue(parameters, x => x.Name))
        {
        ...
        if (PropertyValidator.AllHaveValue(parameters, x => x.Page, x => x.PageSize))
        {
